...
import { todoFormAction, Todo, btnSettings } from "../../../types/index.js";

export const TodoForm = (params: any) => {
    ...
    if(action == "update") options.push("done")
    
    options.forEach(option => {
        const optionTag = document.createElement('option');
        optionTag.value = option;
        optionTag.innerText = option;
        if(todo && option === todo.state) optionTag.setAttribute('selected', 'selected');
        todoStateSelect.appendChild(optionTag)
    })

    const getOption = (where?: "handler") => {
    ...
    const btnAddSettings: btnSettings = {
        icon: null,
        text: action,
        func: handlerEvent,
        styles: ['btn', 'btn-form']
